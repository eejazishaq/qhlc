...
CREATE POLICY "Users can insert own exams" ON user_exams
    FOR INSERT WITH CHECK (user_id = auth.uid());

CREATE POLICY "Admins can view all user exams" ON user_exams
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE id = auth.uid() 
...
-- Exams policies
CREATE POLICY "All authenticated users can view active exams" ON exams
...
-- Gallery policies
CREATE POLICY "Gallery is viewable by all authenticated users" ON gallery
