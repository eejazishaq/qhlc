-- QHLC Setup Initial Data Script (Fixed)
...
DO $$
BEGIN
    -- Add unique constraint to regions.code if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'regions_code_unique' 
        AND conrelid = 'regions'::regclass
    ) THEN
        ALTER TABLE regions ADD CONSTRAINT regions_code_unique UNIQUE (code);
    END IF;
    
    -- Add unique constraint to areas.code if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'areas_code_unique' 
        AND conrelid = 'areas'::regclass
    ) THEN
        ALTER TABLE areas ADD CONSTRAINT areas_code_unique UNIQUE (code);
    END IF;
    
    -- Add unique constraint to countries.code if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'countries_code_unique' 
